...
const BIN_PATH = getenv("PODKOP_BIN") || constants.PODKOP_BIN || "/usr/bin/podkop-plus";
const SERVICE_INIT = getenv("PODKOP_SERVICE_INIT") || constants.PODKOP_SERVICE_INIT || "/etc/init.d/podkop-plus";
...
const COMPONENT_JOB_DIR = getenv("UPDATES_JOB_DIR") || getenv("PODKOP_UI_COMPONENT_ACTION_DIR") || RUNTIME_STATE_DIR + "/component-actions";
...
const SB_MANAGED_SERVICE_MARKER = getenv("SB_MANAGED_SERVICE_MARKER") || constants.SB_MANAGED_SERVICE_MARKER || "Podkop Plus managed sing-box service for binary variants";

let tmp_dir = "";
let lock_held = false;
let podkop_was_running = false;
let podkop_stopped_for_sing_box_change = false;
...
function cleanup_stale_tmp_files() {
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "d", "-name", "podkop-plus-updates.*", "-mmin", "+" + as_string(TMP_STALE_TTL_MINUTES), "-exec", "rm", "-rf", "{}", "+" ]);
    command_success_from_args([ "find", "/tmp", "-maxdepth", "1", "-type", "f", "(", "-name", "podkop-plus-updates-command.*", "-o", "-name", "podkop-plus-updates-http.*", ")", "-mmin", "+" + as_string(TMP_FILE_STALE_TTL_MINUTES), "-delete" ]);
}
...
function init_tmp_dir() {
    ...
    tmp_dir = trim(command_output_from_args([ "mktemp", "-d", "/tmp/podkop-plus-updates.XXXXXX" ]));
    if (tmp_dir == "") {
        tmp_dir = "/tmp/podkop-plus-updates." + owner_pid();
...
function make_tmp_file(prefix) {
    init_tmp_dir();
    let base = tmp_dir != "" ? tmp_dir + "/" + as_string(prefix) + ".XXXXXX" : "/tmp/podkop-plus-updates-" + as_string(prefix) + ".XXXXXX";
...
function restart_podkop_after_failed_sing_box_change() {
    if (!podkop_stopped_for_sing_box_change || !podkop_was_running || !file_exists(SERVICE_INIT))
        return;
...
function action_fail(component, action, message, current_version, latest_version, status, release_url) {
    ...
    restart_podkop_after_failed_sing_box_change();
...
function run_logged(description, command) {
    ...
    if (output_file == "")
        output_file = "/tmp/podkop-plus-updates-command." + owner_pid();
...
function latest_podkop_release_json() {
    let parts = split(PODKOP_RELEASE_REPO, "/");
    if (length(parts) != 2 || as_string(parts[0]) == "" || as_string(parts[1]) == "")
        return "";
    return fetch_github_release_json(parts[0], parts[1]);
}

function latest_podkop_version() {
    let response = latest_podkop_release_json();
    if (response == "")
        return "";
    return trim(helper_output_input(response, "object-get-default", [ "tag_name", "" ]));
}

function fetch_podkop_latest_release_metadata() {
    let response = latest_podkop_release_json();
    if (response == "")
        return "";
...
function write_podkop_latest_version_cache(value, timestamp) {
    if (as_string(value) == "")
        return;
    write_file("/tmp/podkop-plus.latest-version.cache", as_string(value) + "\n" + as_string(timestamp) + "\n");
}
...
function clear_version_caches() {
    remove_file("/tmp/podkop-plus.latest-version.cache");
    remove_file(SYSTEM_INFO_CACHE_FILE);
    remove_file("/tmp/podkop-plus/system-info.json");
}
...
function restart_podkop_after_successful_change() {
    if (!file_exists(SERVICE_INIT))
        return;
    if (!podkop_was_running) {
        updates_log("Podkop Plus was not running before component change; restart skipped");
        prepare_sing_box_service_disabled();
        return;
    }
    run_logged("Restarting Podkop Plus after successful component change", command_from_args([ SERVICE_INIT, "restart" ]));
}

function stop_podkop_before_sing_box_change() {
    if (podkop_stopped_for_sing_box_change)
        return;
    podkop_stopped_for_sing_box_change = true;

    if (podkop_was_running && file_exists(SERVICE_INIT))
        run_logged("Stopping Podkop Plus before sing-box package change", command_from_args([ SERVICE_INIT, "stop" ]));

    if (podkop_was_running && file_exists(BIN_PATH))
...
function wait_podkop_running_after_sing_box_change() {
    ...
    let waited = 0;
    while (waited < 60) {
        if (podkop_status_running_with_timeout()) {
...
function install_zapret_like(component, action, runtime_module, resolve_fn, label) {
    ...
    restart_podkop_after_successful_change();
...
function install_sing_box_extended_package(action) {
    ...
    stop_podkop_before_sing_box_change();
    prepare_sing_box_package_service_install();
    ...
    if (current_variant == "extended" || current_variant == "extended-compressed") {
        if (file_exists("/usr/bin/sing-box")) {
            backup_binary = "/usr/bin/sing-box.podkop-backup." + owner_pid();
        ...
        if (file_exists("/usr/lib/libcronet.so")) {
            cronet_touched = true;
            backup_cronet = "/usr/lib/libcronet.so.podkop-backup." + owner_pid();
    ...
    write_sing_box_variant_state("extended", new_version);
    restart_podkop_after_successful_change();
    if (!wait_podkop_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_package_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, package_file, cronet_touched)) {
            ...
            action_fail("sing_box", action, "sing-box-extended package was installed but Podkop Plus did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, "sing-box-extended package was installed but Podkop Plus did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_sing_box_extended(action, compressed) {
    ...
    if (!wait_podkop_running_after_sing_box_change()) {
        ...
        if (restore_sing_box_after_failed_extended_install(current_variant, backup_binary, backup_cronet, previous_marker, previous_version_state, archive_file, cronet_touched)) {
            remove_file(backup_binary);
            remove_file(backup_cronet);
            action_fail("sing_box", action, label + " was installed but Podkop Plus did not start cleanly; previous sing-box variant was restored", current_version, latest_version);
        }
        action_fail("sing_box", action, label + " was installed but Podkop Plus did not start cleanly and previous sing-box variant could not be restored", current_version, latest_version);
    }
...
function install_package_sing_box(action, tiny) {
    ...
    if (!wait_podkop_running_after_sing_box_change()) {
        clear_version_caches();
        action_fail("sing_box", action, (tiny ? "Tiny" : "Stable") + " sing-box was installed, but Podkop Plus did not start cleanly", new_version, latest_version);
    }
    clear_version_caches();
...
function check_podkop_plus() {
    let metadata = fetch_podkop_latest_release_metadata();
    ...
    if (latest_version == "unknown")
        action_fail("podkop", "check_update", "Failed to check Podkop Plus updates", PODKOP_VERSION, latest_version);

    write_podkop_latest_version_cache(latest_version, now_seconds());
    if (!helper_success("podkop-release-version-valid", [ PODKOP_VERSION ])) {
        updates_log("Podkop Plus current version is not a release version (" + PODKOP_VERSION + ")");
        action_success("podkop", "check_update", "Installed version is newer than release", PODKOP_VERSION, latest_version, 0, "dev", release_url);
    }

    let compare = trim(helper_output("podkop-release-version-compare", [ PODKOP_VERSION, latest_version ]));
    if (compare == "")
        action_fail("podkop", "check_update", "Failed to compare Podkop Plus versions", PODKOP_VERSION, latest_version);
    let status = status_from_compare(int(compare));
    if (status == "")
        action_fail("podkop", "check_update", "Failed to compare Podkop Plus versions", PODKOP_VERSION, latest_version);
    if (status == "latest") {
        updates_log("Podkop Plus is already up to date (" + PODKOP_VERSION + ")");
        action_success("podkop", "check_update", "Latest version is installed", PODKOP_VERSION, latest_version, 0, status, release_url);
    }
    if (status == "outdated") {
        updates_log("Podkop Plus update found: " + PODKOP_VERSION + " -> " + latest_version);
        action_success("podkop", "check_update", "Update is available", PODKOP_VERSION, latest_version, 0, status, release_url);
    }
    updates_log("Podkop Plus installed version is newer than upstream release: " + PODKOP_VERSION + " -> " + latest_version);
    action_success("podkop", "check_update", "Installed version is newer than release", PODKOP_VERSION, latest_version, 0, status, release_url);
}

function resolve_podkop_plus_release(latest_version) {
    let release_json = latest_podkop_release_json();
    if (release_json == "")
        return null;
    let asset_ext = is_apk() ? "apk" : "ipk";
    let i18n_required = pkg_is_installed("luci-i18n-podkop-plus-ru") ? "1" : "0";
    let plan = trim(helper_output_input(release_json, "podkop-plus-release-plan", [ latest_version, asset_ext, i18n_required ]));
...
function install_podkop_plus() {
    let latest_version = latest_podkop_version();
    if (latest_version == "")
        latest_version = "unknown";
    if (latest_version == "unknown")
        action_fail("podkop", "install", "Failed to resolve Podkop Plus release", PODKOP_VERSION, latest_version);

    write_podkop_latest_version_cache(latest_version, now_seconds());
    init_tmp_dir() || action_fail("podkop", "install", "Failed to create temporary directory", PODKOP_VERSION, latest_version);
    updates_log("Resolving Podkop Plus release " + latest_version + " packages");
    let release = resolve_podkop_plus_release(latest_version);
    if (release == null)
        action_fail("podkop", "install", "Failed to resolve Podkop Plus release packages", PODKOP_VERSION, latest_version);
    ...
    if (!download_with_retry(release.backend_url, backend_file, release.backend_name) ||
        ...
        action_fail("podkop", "install", "Failed to download Podkop Plus release packages", PODKOP_VERSION, latest_version);

    if (!run_logged("Installing LuCI app package " + release.app_name, pkg_install_files_command([ app_file ])))
        action_fail("podkop", "install", "Failed to install LuCI app package", PODKOP_VERSION, latest_version);
    if (i18n_file != "" && !run_logged("Installing LuCI Russian i18n package " + release.i18n_name, pkg_install_files_command([ i18n_file ])))
        action_fail("podkop", "install", "Failed to install LuCI Russian i18n package", PODKOP_VERSION, latest_version);
    if (!run_logged("Installing Podkop Plus package " + release.backend_name, pkg_install_files_command([ backend_file ])))
        action_fail("podkop", "install", "Failed to install Podkop Plus package", PODKOP_VERSION, latest_version);
    ...
    restart_podkop_after_successful_change();
    clear_version_caches();
    let new_version = installed_package_version("podkop-plus");
    if (new_version == "")
        new_version = latest_version;
    updates_log("Podkop Plus updated to " + new_version);
    action_success("podkop", "install", "Podkop Plus has been installed", new_version, latest_version, 1, "latest", release.release_url);
}
...
function component_action(component, action) {
    ...
    capture_podkop_running_state();

    if (component == "podkop" && action == "check_update")
        check_podkop_plus();
    else if (component == "podkop" && action == "install")
        install_podkop_plus();
...
else if (mode == "latest-podkop-release-json")
    print(latest_podkop_release_json());
else if (mode == "latest-podkop-version")
    print(latest_podkop_version(), "\n");
else if (mode == "podkop-release-metadata")
    print(fetch_podkop_latest_release_metadata(), "\n");
else {
    warn("Usage: components/action.uc <component-action|latest-podkop-version|podkop-release-metadata> ...\n");
    exit(1);
}
