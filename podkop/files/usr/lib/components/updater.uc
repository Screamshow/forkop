...
function release_asset_pair_by_prefix(release, prefix, ext) {
    for (let asset in array_or_empty(release.assets)) {
        if (type(asset) != "object")
            continue;
...
function podkop_plus_release_plan(latest_version, asset_ext, i18n_required) {
    let release = object_or_empty(read_stdin_json());

    if (as_string(release.tag_name || "") != as_string(latest_version))
        exit(1);

    let backend = release_asset_pair_by_prefix(release, "podkop-plus", asset_ext);
    let app = release_asset_pair_by_prefix(release, "luci-app-podkop-plus", asset_ext);
    if (backend == null || app == null)
        exit(1);

    let i18n = { name: "", url: "" };
    if (arg_bool(i18n_required)) {
        i18n = release_asset_pair_by_prefix(release, "luci-i18n-podkop-plus-ru", asset_ext);
        if (i18n == null)
            exit(1);
    }
...
else if (mode == "podkop-plus-release-plan")
    podkop_plus_release_plan(ARGV[1], ARGV[2], ARGV[3]);
