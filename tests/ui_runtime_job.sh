...
grep -Fq 'reason == "manual-ui-restart" ? [ BIN_PATH, "manual_restart" ] : [ SERVICE_INIT, action ]' "$UI_UC" ||
  fail "UI restart must use the guarded manual_restart path while other actions use init.d"
