...
# Channel selection is installer-owned and is deliberately persisted once.
# Other configuration ownership must remain in the ucode helper/runtime.
uci_lines="$(awk '
  /^persist_release_channel\(\)/ { allowed = 1 }
  allowed && /^}/ { allowed = 0; next }
  !allowed && /(^|[;&|[:space:]])uci[[:space:]]+-q|command_exists[[:space:]]+uci|\/usr\/bin\/uci/ { print }
' "$INSTALLER")"
if [ -n "$uci_lines" ]; then
  fail "install.sh must not own UCI reads/writes outside release-channel persistence"
fi
...
grep -Fq 'tiny)
            action="install_tiny"' "$INSTALLER" ||
  fail "automatic tiny selection must install the tiny component action"

# B: a supported installed package must be preserved and its component action skipped.
existing_variant="$(
  legacy_binary_managed_sing_box_present() { return 1; }
  sing_box_is_present() { return 0; }
  msg() { :; }
  SING_BOX_INSTALL_VARIANT="unexpected"
  . "$select_sing_box_block"
  select_sing_box_installation
  printf '%s' "$SING_BOX_INSTALL_VARIANT"
)"
[ -z "$existing_variant" ] || fail "existing sing-box must not be replaced during normal installation"
