...
type RuntimeUiStateListener = (uiState: Podkop.UiState) => void;

let runtimeUiStateRefreshPromise: Promise<Podkop.UiState | undefined> | null =
  null;
let lastRuntimeUiStateRefreshAt = 0;
let lastRuntimeUiState: Podkop.UiState | undefined;
...
function hasRunningAction(uiState: Podkop.UiState) {
...
function notifyRuntimeUiStateListeners(uiState: Podkop.UiState) {
...
export async function refreshRuntimeUiState({
  force = false,
}: { force?: boolean } = {}): Promise<Podkop.UiState | undefined> {
  ...
  const promise = PodkopShellMethods.getUiState()
    .then((response) => {
      if (!response.success) {
        return undefined;
      }
